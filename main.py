...
# IMPORTANT: Using pin 23 as you've confirmed.
...
COOLDOWN_TIME_SECONDS = 15 # Ensure this is longer than your actual process time
...
# --- Function to Generate Poem with Gemini via curl (Unchanged) ---
...
def run_poetry_printer(channel):
    ...
    if GPIO.input(channel) == GPIO.HIGH: # If it's HIGH, it's not actually pressed (assuming PUD_UP)
        print(f"\n[DEBUG] Callback triggered for GPIO {channel} but pin is currently HIGH. Ignoring false trigger.")
        return

    # If we get here, the press is valid, so update the last action time
    last_poetry_action_time = current_time
    # --- END SOFTWARE DEBOUNCE LOGIC ---

    print(f"\n[DEBUG] Callback triggered for GPIO {channel}! Initiating poetry process.")
    # Turn off LED while processing to indicate busy state
    ...
    # Print the ready message again at the end of the process
...
if __name__ == "__main__":
    # Variable to store the previous button state to detect changes
    ...
    try:
        ...
        while True:
            ...
            # Only print the button state if it has changed
            if current_button_state != last_displayed_button_state:
                print(f"Button state: {'LOW (Pressed)' if current_button_state == GPIO.LOW else 'HIGH (Not Pressed)'}")
